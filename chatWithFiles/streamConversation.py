...
import uuid
import time
from database import save_message, get_or_create_conversation_history  # 导入数据库操作方法
import threading
import uvicorn
...
# 定期清空会话历史
def cleanup_history():
    """定期清空answer_history和match_history"""
    global answer_history, match_history
    answer_history.clear()
    match_history.clear()
    print("清空了所有对话和匹配历史")

# 定时任务：每小时清空一次
def schedule_cleanup(interval=300):
    """每小时清空一次历史记录"""
    cleanup_history()  # 立即清空一次
...
def match_pdfs_with_llm(request: QueryRequest):
    """第一次调用：大模型匹配PDF文件，加入缓存"""
    query = request.query
    conversation_id = request.conversation_id
    # 获取当前对话历史（如果不存在则创建）
    messages = get_or_create_conversation_history(conversation_id, "match",match_history)
    #messages = match_history.setdefault(conversation_id, [])

    # 1. 读取目录文件
    files_info = read_pdf("现行制度体系.pdf")  # 读取目录文件
    system_prompt = f"""
    ...
    save_message("match",conversation_id, "user", query)
    ...
    try:
        ...
        try:
            ...
            save_message("match", conversation_id, "assistant", collected_answer)
            match_history[conversation_id] = messages


        # 触发回答阶段（无论是否解析成功）
        if data.get("matched_files"):
            for answer_chunk in answer_with_llm(data, request):
...
@app.post("/chat")
def chat_with_files(request: QueryRequest):
...
if __name__ == "__main__":
    ...
    cleanup_thread.daemon = True  # 确保在主线程退出时清理线程会被终止
