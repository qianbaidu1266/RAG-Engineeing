...
def initialize_rag_system():
    ...
    loader = SimpleDirectoryReader(
        input_dir=ModelConfig.DATA_DIR,
        required_exts=[".pdf"],  # 仅加载 PDF 文件
        recursive=True,  # 递归子目录
        num_files_limit=100  # 限制最大文件数
    )

    # 创建索引
    index = VectorStoreIndex.from_documents(loader.load_data(), show_progress=True)
    # 保存索引到磁盘
